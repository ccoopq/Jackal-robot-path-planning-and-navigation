...
GoalPublisherNode::GoalPublisherNode() : tf2_listener_(tf2_buffer_)
{
  ...
  this->pub_relative_heading_error_ = nh_.advertise<std_msgs::Float32>("/me5413_world/relative/heading_error", 1);
	this->pub_pose_world_robot_ = nh_.advertise<geometry_msgs::Pose>("/me5413_world/pose_world_robot", 1); //***
  ...
  this->sub_box_markers_ = nh_.subscribe("/gazebo/ground_truth/box_markers", 1, &GoalPublisherNode::boxMarkersCallback, this);
	...
	this->sub_box_path_ = nh_.subscribe("/search_world_goal", 1, &GoalPublisherNode::boxPathCallback, this); // ***
	this->sub_box_flag_ = nh_.subscribe("/detect_box_flag", 1, &GoalPublisherNode::boxFlagCallback, this); // ***
  ...
  this->box_init_flag = 0; //***
	this->search_init_flag = 0;
	this->box_flag_ = 0;
};
...
void GoalPublisherNode::timerCallback(const ros::TimerEvent&)
{
  ...
  // std::cout << "this->pose_map_robot_ = " << this->pose_map_robot_ << std::endl;
  // std::cout << "this->pose_map_goal_ = " << this->pose_map_goal_ << std::endl;
  // std::cout << "this->pose_world_robot_ = " << this->pose_world_robot_ << std::endl;
  // std::cout << "this->pose_world_goal_ = " << this->pose_world_goal_ << std::endl;
  ...
  this->pub_relative_heading_error_.publish(this->relative_heading_error_);
	this->pub_pose_world_robot_.publish(this->pose_world_robot_); //***

  //if (this->goal_type_ == "box")
  //{
   // this->pub_goal_.publish(searchForNextTarget()); //***
  //}

  return;
};

void GoalPublisherNode::boxPathCallback(const geometry_msgs::PoseStamped::ConstPtr& next_path) //*** 检测到box，接收path
{
	geometry_msgs::PoseStamped box_path; // 这个是在world坐标系下的
	geometry_msgs::TransformStamped transform_map_world = this->tf2_buffer_.lookupTransform(this->map_frame_, this->world_frame_, ros::Time(0));
	tf2::doTransform(next_path->pose, box_path.pose, transform_map_world);
	this->box_path_ = box_path;
	return;
}

void GoalPublisherNode::boxFlagCallback(const std_msgs::Int32::ConstPtr& flag) //*** 是否检测到box
{
	this->box_flag_ = flag->data;
	return;
}

geometry_msgs::PoseStamped GoalPublisherNode::generateNewSearchPoint() //*** 生成新搜寻点函数
{
	// 在正方形搜索范围内随机生成目标点
      geometry_msgs::PoseStamped search_world_goal_;
			geometry_msgs::PoseStamped search_map_goal_;
			search_map_goal_.header.stamp = ros::Time::now();
      search_map_goal_.header.frame_id = map_frame_;
			search_world_goal_.header.stamp = ros::Time::now();
      search_world_goal_.header.frame_id = map_frame_;
      search_world_goal_.pose.position.x = 12.9 + (rand() % (5)) - 3; // x: = +2.0 ~ -3.0
      search_world_goal_.pose.position.y = 2.2 + (rand() % (7)) - 7; // y: = 0 ~ -7
      search_world_goal_.pose.position.z = 0.0;
			search_world_goal_.pose.orientation.x = 0;
			search_world_goal_.pose.orientation.y = 0.0;
			search_world_goal_.pose.orientation.z = 0.0;
			search_world_goal_.pose.orientation.w = 1;
			// 转到map坐标系
			geometry_msgs::TransformStamped transform_map_world = this->tf2_buffer_.lookupTransform(this->map_frame_, this->world_frame_, ros::Time(0));
			tf2::doTransform(search_world_goal_.pose, search_map_goal_.pose, transform_map_world);
			return search_map_goal_;
}

geometry_msgs::PoseStamped GoalPublisherNode::searchForNextTarget() //*** box搜寻函数
{
  // 设置起始点
  geometry_msgs::PoseStamped start_goal;
	geometry_msgs::Pose start_world_goal;
  start_goal.header.stamp = ros::Time::now();
  start_goal.header.frame_id = map_frame_;
  start_goal.pose.position.x = 12.9;
  start_goal.pose.position.y = 1.9;
  ...
  start_goal.pose.orientation.w = 1.0;
	...
	transform_world_map = this->tf2_buffer_.lookupTransform(this->world_frame_, this->map_frame_, ros::Time(0));
	tf2::doTransform(start_goal.pose, start_world_goal, transform_world_map);
  const std::pair<double, double> error_init = calculatePoseError(this->pose_world_robot_, start_world_goal); // 计算与起始点的误差
	std::cout << "error_init.first  = " << error_init.first  << std::endl;

  if (error_init.first  > 0.15 && !this->box_init_flag) { // case 1: 没到起始点
		std::cout << "----------------------- not yet in start ------------------------------" << std::endl;
		return start_goal;
  }
  else { // case 2: 到了起始点，开始搜寻
		std::cout << "----------------------- start searching ------------------------------" << std::endl;
    this->box_init_flag = 1; // 标记已经到过起始点
		// geometry_msgs::PoseStamped search_map_goal; // 搜寻点
		if (!this->search_init_flag) { // 还没有搜寻过
			this->search_map_goal = generateNewSearchPoint();
			this->search_init_flag = 1; // 标记第一次生成了搜寻点
		}

    if (this->box_flag_) { // 搜寻到目标box
			return this->box_path_;
    }
    else { // 没有搜寻到目标box, 开始多点随机搜寻
		 geometry_msgs::Pose search_world_goal;
			tf2::doTransform(this->search_map_goal.pose, search_world_goal, transform_world_map);
      const std::pair<double, double> error_search = calculatePoseError(this->pose_world_robot_, search_world_goal); // 计算与搜寻点的误差
			std::cout << "error_search.first  = " << error_search.first  << std::endl;

      if (error_search.first > 0.7) { // 还没到当前搜寻点
				return this->search_map_goal;
			}
			else { // 已经到了当前搜寻点，需要更新搜寻点
				this->search_map_goal = generateNewSearchPoint();
				return this->search_map_goal;
			}
    }
...
{ 
  ...
  if (this->goal_type_ != "box")
  {
    this->pub_goal_.publish(P_map_goal);
		...
		P_map_goal.pose.position.x += 0.5;
		this->pub_goal_.publish(P_map_goal);
	}
