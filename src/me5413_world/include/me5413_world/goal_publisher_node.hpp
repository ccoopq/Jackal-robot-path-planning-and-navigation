...
class GoalPublisherNode
{
 ...
 private:
  ...
  geometry_msgs::PoseStamped generateNewSearchPoint(); //***
  geometry_msgs::PoseStamped searchForNextTarget();
  void boxPathCallback(const geometry_msgs::PoseStamped::ConstPtr& next_path);
  void boxFlagCallback(const std_msgs::Int32::ConstPtr& flag);
  ...
  int box_init_flag; //***
  int search_init_flag;
  geometry_msgs::PoseStamped search_map_goal;
  int box_flag_;
  ros::Subscriber	sub_box_path_;
	ros::Subscriber sub_box_flag_;
  geometry_msgs::PoseStamped box_path_;
  ros::Publisher pub_pose_world_robot_;
};
